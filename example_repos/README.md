...
Contains the **full source code** including:
- All necessary files to build and test
- Extra/unused code (functions, files) that could be removed
- Must pass `make test`
...
1. **test_example_repos.py**: Copies `before/`, deletes all files, runs `boil --hard make test`, verifies boil can restore the codebase
...
2. Ensure `make test` passes in `before/`
